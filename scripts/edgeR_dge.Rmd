...
ggplot(cbind(as.data.frame(mds$cmdscale.out), dge$samples) %>%
...
log-fold change below log2(3)), and thus we test against the null hypothesis that 
the log-fold change between the compared groups is less than log2(3), using the 
...
edgeR_res <- lapply(contrasts, function(cm) {
    qlf <- edgeR::glmTreat(qlfit, contrast = cm, lfc = log2(3))
